...
/// If `secret` is Some, adds an `X-TxWatch-Signature: sha256=<hmac>` header computed over the request body.
...
mod tests {
    ...
    async fn signature_header_present_when_provided() {
        ...
        let sig = requests[0].headers.get("x-txwatch-signature")
            .expect("X-TxWatch-Signature header must be present");
    ...
    async fn signature_header_absent_when_not_provided() {
        ...
        assert_eq!(requests.len(), 1);
        assert!(
            !requests[0].headers.contains_key("x-txwatch-signature"),
            "X-TxWatch-Signature header must not be present when secret is None"
        );
    }
